...
## ⚙️ Cara Mengganti Nomor WhatsApp Cabang

Semua nomor WhatsApp cabang dikelola di berkas [`js/data.js`](file:///d:/ALL%20CODE/horus/js/data.js). Buka berkas tersebut dan ubah bagian `branches`:
...
branches: [
  {
    ...
    whatsapp: "6281234567890", // Ganti dengan nomor WhatsApp aktif (gunakan kode negara 62 tanpa tanda +)
...
Cukup buka berkas `index.html` langsung di browser, atau gunakan ekstensi Live Server / HTTP server lokal:
```bash
npx serve .
```
...
3. Vercel akan otomatis mengenali berkas `index.html` dan `vercel.json` tanpa perlu perintah build!
